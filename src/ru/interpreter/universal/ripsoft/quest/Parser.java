...
//           return in;
//    }
    public String parseAllUnknownVariables(String in){
     String temp = in.replace("+", ";");
            temp = temp.replace("-", ";");
            temp = temp.replace("*", ";");
            temp = temp.replace("/", ";");
            ...
            temp = temp.replace(">", ";");
            temp = temp.replace("<", ";");
             System.out.println("parseAllUnknownVariables: "+temp);
            HashMap<String,String> hm = getCore().getVariables().getVariablesHash();
            LinkedHashMap<String,Integer> lhm = getCore().getInventory().getInventoryHash();
           String[] t = temp.split(";");
           for (String ss :t){
           ...
           //   for (String s :tt){
                   String v = ss.trim();
                   if (!v.isEmpty()&&!v.equals(" ")&&v.indexOf("true")==-1&&v.indexOf("false")==-1&&
                       !isNum(v)&&!lhm.containsKey(v.replace("inv_", ""))&&!lhm.containsKey(v.replace("Inv_", ""))&&
                       !hm.containsKey(v)){
                       if (v.toLowerCase().startsWith("inv_")){
                        getCore().getInventory().addInv(0, v.substring(4, v.length()));
                        System.out.println("AddNEWInventory: "+v.trim());
                       }else{
                        getCore().getVariables().addVariable(v.trim(), "0");
                        System.out.println("AddNEWVariable: "+v);
